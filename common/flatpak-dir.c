...
_flatpak_dir_find_new_flatpakrepos (FlatpakDir *self, OstreeRepo *repo)
{
  ...
  if (self->user)
    {
      config_dir_str = g_build_filename (FLATPAK_CONFIGDIR, FLATPAK_REMOTES_DIR, NULL);
  ...
  for (int i = 0; remotes[i] != NULL; i++)
    {
      ...
      if (config == NULL || !g_key_file_has_group (config, group, NULL))
        continue;

      if (g_key_file_get_boolean (config, group, FLATPAK_CENTRAL_KEY_LOCKDOWN_MANAGED_REMOTE, NULL))
        continue;
                                        ...
                                        GError      **error)
{
  ...
  config = ostree_repo_get_config (self->repo);
  if (config == NULL || !g_key_file_has_group (config, group))
    return FALSE;

  return g_key_file_get_boolean (config, group, FLATPAK_CENTRAL_KEY_LOCKDOWN_MANAGED_REMOTE, NULL);
}
